...
public class ReportsUI {
	...
	private ReportsUI(MainUI mainUI, ParkingServerInterface parking) {
		this.mainUI = mainUI;
		this.parking = parking;
	}

	public static ReportsUI getInstance(MainUI mainUI, ParkingServerInterface parking) {
		if (instance == null) {
			instance = new ReportsUI(mainUI, parking);
		}
		return instance;
	}
			...
			if (reply == JOptionPane.OK_OPTION) {
				...
				if (monthYearStr.matches("[0-9]{2}-[0-9]{4}")) {
					...
					if (isReportPopulated) {
						...
						JOptionPane.showMessageDialog(mainUI, "Enter valid month in the past in format mm-yyyy.", "Error", JOptionPane.ERROR_MESSAGE);
						dailyReportGetMonthTxt.setText("");
					}
				} else {
